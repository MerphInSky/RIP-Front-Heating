...
import { getComponent } from "../../modules/componentsApi.ts";
...
export default function Breadcrumbs() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void getComponent(id).then((d) => {
      if (cancelled) return;
      if (d) setComponentTitle(d.title);
      else {
        const mock = COMPONENTS_MOCK.find((x) => x.component_id === id);
        setComponentTitle(mock?.title ?? `Компонент ${id}`);
      }
    });
    return () => {
      cancelled = true;
