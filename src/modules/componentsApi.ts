...
export interface HeatingDetailResponse {
  heating: HeatingJSON;
  components: HeatingComponentJSON[];
}
...
export async function getHeatingCart(): Promise<HeatingCart> {
  try {
    const res = await fetch("/api/heatings/cart", {
...
export async function listComponents(params?: { title?: string }): Promise<Component[]> {
  try {
    ...
    const res = await fetch(path, { headers: { Accept: "component/json" } });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return await res.json();
  } catch {
    return [];
...
export async function getHeating(
  ...
  try {
    const res = await fetch(`/api/heatings/${id}`, { headers });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return await res.json();
  } catch {
    return null;
...
export async function addComponentToHeating(
  ...
  try {
    const res = await fetch(`/api/heating_components/add/${componentId}`, {
      method: "POST",
      headers: {
        Accept: "heating/json",
...
export async function editComponentInHeating(
  ...
  try {
    const res = await fetch(`/api/heating_components/${componentId}/${heatingId}`, {
      method: "PUT",
      headers: {
